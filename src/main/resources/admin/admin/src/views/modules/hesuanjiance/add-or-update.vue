<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.jiancefengmian" label="检测封面" prop="jiancefengmian">
          <file-upload
          tip="点击上传检测封面"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.jiancefengmian?ruleForm.jiancefengmian:''"
          @change="jiancefengmianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.jiancefengmian" label="检测封面" prop="jiancefengmian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.jiancefengmian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="检测类型" prop="jianceleixing">
          <el-select :disabled="ro.jianceleixing" v-model="ruleForm.jianceleixing" placeholder="请选择检测类型">
            <el-option
...
export default {
	...
	faburiqi : false,
      ...
      ruleForm: {
        ...
        faburiqi: '',
          ...
          jianceleixingOptions: [],
      rules: {
          jiancemingcheng: [
                { required: true, message: '检测名称不能为空', trigger: 'blur' },
          ],
          jiancefengmian: [
          ],
          jiancedidian: [
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.jiancemingcheng = true;
          ...
          if(o=='jiancefengmian'){
            this.ruleForm.jiancefengmian = obj[o];
	    this.ro.jiancefengmian = true;
            continue;
          }
          if(o=='jiancedidian'){
	...
	if(this.ruleForm.jiancefengmian!=null) {
		this.ruleForm.jiancefengmian = this.ruleForm.jiancefengmian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
    ...
    jiancefengmianUploadChange(fileUrls) {
	this.ruleForm.jiancefengmian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
